...
/*
 * CS 325 - Digital Assignment 9
 *		Title: Spartan Showdown (iteration)
 *
 */
...
class MyScene extends Phaser.Scene {
    ...
    create() {
		...
		this.curTrap = '';		// current trap selected (spikes,slime,bomb)
		this.curTraps = [];		// current traps placed
		this.curBombs = [];		// current bombs placed
		...
		let bombX = 585;
		this.bombTrap = this.physics.add.sprite(bombX,580, 'bomb').setInteractive();
		this.bombTrap.setScale(bombScale);
		this.bombTrap.on('pointerdown', this.bombSelected, this);
		...
		this.bombHeader = this.add.text(bombX-25, heightHeader, 
			"BOMB: "+bombCost, headerStyle);
		...
		let UIComps = [this.instr1,this.instr2,this.instr3,timeHeader,
			moneyHeader,this.spikesHeader,this.slimeHeader,this.bombHeader,
			this.spikeTrap,this.slimeTrap,this.bombTrap,time,money,
    ...
    update() {
		...
		if(isRunning) {
			...
			if(this.tower1.data.get('health') > 0 || 
				...
				// check if a bomb should detonate
				for(var i=0; i<this.curTraps.length; i++) {
					if(this.curTraps[i].data.get('type') === 'bomb') {
						if(curTime - this.curTraps[i].data.get('time') > this.curTraps[i].data.get('duration')) {
							// detonate bomb
							this.bombExplode(this.curTraps[i]);
							i--;
						}
					}
					else continue;
	...
	towerHit(enemy, tower) {
		...
		let damage = enemy.data.get('damage');
		tower.setData('health', tower.data.get('health') - damage);
	...
	spawnEnemy() {
		...
		let isStrong = Math.floor(Math.random() * 5) === 0;	// 20% chance of strong
		...
		if(isStrong) {
			damage = 3;
			enemy.tint = 0xee0000	// tinted red
		}
	...
	spikesSelected() {
		...
		this.bombHeader.setColor("#00CC00");
	}
	
	// process slime trap selection
	slimeSelected() {
		this.curTrap = 'slime';
	...
	// process bomb trap selection
	bombSelected() {
		this.curTrap = 'bomb';
		console.log("Bomb clicked", this.curTrap);
		this.bombHeader.setColor("#FF0000");
	...
	bgClicked() {
		...
		else if(this.curTrap === 'bomb' && curMoney>=bombCost) {
			// place bomb
			this.updateMoney(curMoney-bombCost);
			console.log("bomb placed");
			
			let bombTrap = this.physics.add.sprite(x,y, 'bomb');
			bombTrap.setScale(bombScale);
			bombTrap.body.immovable = true;
			bombTrap.setData('type', 'bomb');
			bombTrap.setData('health', -1);
			bombTrap.setData('handler', this.bombExplode);
			bombTrap.setData('time', curTime);
			bombTrap.setData('duration', bombTime);
			
			this.curTraps.push(bombTrap);
			//this.curBombs.push(bombTrap);
		}
	}
	...
	// register bomb explosion
	bombExplode(bomb) {
		console.log("bomb explode");
		let x = bomb.x;
		let y = bomb.y;
		
		// check all enemies distance from bomb
		for(var i=0; i<this.enemyGroups.length; i++) {
			let enemy = this.enemyGroups[i].enemy;
			if(enemy === undefined)
				continue;
			...
			if(Math.sqrt(Math.pow(x-x1,2) + Math.pow(y-y1,2)) <= bombRadius) {
				// enemy hit by bomb
				this.despawnEnemy(enemy);
				i--;
			}
